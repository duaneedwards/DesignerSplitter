...
namespace DesignerSplitter
{
	...
	internal sealed class SplitDesignerFile
	{
		...
		private void MenuItemCallback(object sender, EventArgs e)
		{
			...
			//string title = "SplitDesignerFile";
		...
		private void ExtractDesignerFile()
		{
			...
			var newItemPath = string.Format("{0}\\{1}.Designer.cs", dir, bareName);
			...
			{
				usingStatements.AppendLine("using " + import.Namespace + ";");
			}
			...
			var namespaceName = codeClass.Namespace.FullName;
			...
			toWrite.AppendLine();
			toWrite.AppendLine(string.Format("namespace {0}", namespaceName));
			toWrite.AppendLine("{");
